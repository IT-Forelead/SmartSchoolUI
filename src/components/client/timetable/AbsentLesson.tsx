...
export default function AbsentLesson(props: {
  ...
  setSelectedSubject: React.Dispatch<
    React.SetStateAction<LessonBody | undefined>
  >;
  refetch: <TPageData>(
  ...
  const {
    mutate: changeTeacherLesson,
    isSuccess,
    error,
    isLoading,
  } = useChangeTeacherLesson();
  ...
  function compareLessonBody(lesson: LessonBody) {
    if (
      props.availableLessons?.find((l) => {
        return (
          l?.moment === lesson.moment &&
  ...
  function fillColorSelectedSubject(lesson: LessonBody) {
    if (
      props.selectedSubject?.moment === lesson.moment &&
      props.selectedSubject.groupId === lesson.groupId &&
      props.selectedSubject.subjectId === lesson.subjectId &&
      props.selectedSubject.teacherId === lesson.teacherId &&
